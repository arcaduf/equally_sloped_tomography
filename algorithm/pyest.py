...
def ppft_adj( f ):
    ...
    f_out[:,:] = 1/(n*np.sqrt(2)) * ( f_out[:,:] + f_aux[:,nh:3*nh] ) 
...
def ippft( f , niter=6 , eps=1e-8 ):
    ...
    while it < niter:
        ...
        print('\t\t IPPFT Iter. num. ' , it , '  --->  Error: ' , err ) 
        if err > eps:
            f[:,:] = ppft( pk )
            f[:,:] = np.multiply( M , f )
            ppAdj[:,:] = ppft_adj( f )
...
def fill_pseudo_polar_grid( sino , angles , n , eps=None  ):
    ...
    for i in range( nang ):
        ...
        if pseudo_indeces[ind] <= nfreqq:
            proj_aux[ npixh-npix_oph : npixh+npix_oph+1 ] = sino[i,:]
            fslice[:] = calc_est_slice( proj_aux , pseudo_alphas[ind] )

        else:
            proj_aux[ npixh-npix_oph : npixh+npix_oph+1 ] = sino[i,::-1] 
            fslice[:] = calc_est_slice( proj_aux , pseudo_alphas[ind] )
